  ...
  for(k in bindMode){
    ...
    for(i in 1:M){
      ...
      pdf("selectedFeature_LASSO_feature_selection.pdf",width=6,height=5)
      #par(mfcol=c(1,1))
      par(cex=1.3)
      #par(mar=c(4,4,2,1))
      #par(mgp = c(2, 0.5, 0))
      ...
      pdf("selectedFeature_LASSO_best_lambda.pdf",width=6,height=5)
      #par(mfcol=c(1,1))
      par(cex=1.3)
      #par(mar=c(4,4,2,1))
      #par(mgp = c(2, 0.5, 0))
      ...
      # if(i==1){
      #   write.table(Result,file=paste0("LASSO_finalResult_averagePrediction_",k,".txt"),quote=FALSE,col.names=T,row.names=F,sep='\t')
      # }else{
      #   write.table(Result,file=paste0("LASSO_finalResult_averagePrediction_",k,".txt"),quote=FALSE,col.names=F,row.names=F,sep='\t',append = T)
      # }
  ...
  # 根据混淆矩阵计算特异度、敏感度以及准确度指标
  SE <- sum_TP / (sum_TP + sum_FN) ##也叫召回率Recall
  SP <- sum_TN / (sum_TN + sum_FP)
  ACC <- (sum_TP + sum_TN) / (sum_TP + sum_TN + sum_FP + sum_FN)
  TPR<-SE
  TNR<-SP
  MCC=((sum_TP*sum_TN)-(sum_FP*sum_FN))/sqrt((sum_TP+sum_FP)*(sum_TN+sum_FN)*(sum_TP+sum_FN)*(sum_TN+sum_FP))
  
  alpha=0.5
  PPV=sum_TP/(sum_TP+sum_FP)   #positive predictive value, also called Precision精确率
  Fmeasure = 1/ (alpha*1/PPV + (1-alpha)*1/TPR) #即Fmeasure=2*PPV*SE/(PPV+SE) ##也就是Fmeasure=2*Precision*Recall/(Precision+Recall)
  
  ######################绘制整合全部decision values后的ROC曲线################################################
  model_roc <- roc(ALLtag,as.numeric(ALLprob_values))
  #plot(model_roc,legacy.axes=T, print.auc=T, auc.polygon=T, grid=c(0.1, 0.1),grid.col=c("green", "red"), max.auc.polygon=T,auc.polygon.col="orange", print.thres=T,main='ROC curve, kernel = radial')
  #jpeg("ROC.jpeg",width=600*6,height=400*6,res=72*8)
  pdf("selectedFeature_LASSO_ROC_micro_averagePrediction.pdf",width=6,height=5)
  #par(mfcol=c(1,1))
  par(cex=1.3)
  #par(mar=c(4,4,2,1))
  #par(mgp = c(2, 0.5, 0))
  plot(model_roc,legacy.axes=T,print.auc=T, auc.polygon=T, max.auc.polygon=T,auc.polygon.col="orange", print.thres=F,print.thres.cex=1,print.auc.cex=1)
  dev.off()
  
  
  
  
  scores <- data.frame(predictions=as.numeric(ALLprob_values), labels=as.numeric(as.character(ALLtag)))
  pr <- pr.curve(scores.class0=scores[scores$labels==1,"predictions"],
                 scores.class1=scores[scores$labels==0,"predictions"],
                 curve=T,max.compute=T, min.compute=T, rand.compute=T)
  
  pr$auc.integral=round(pr$auc.integral,3)
  pdf("selectedFeature_LASSO_PRC_micro_averagePrediction.pdf",width=5,height=5)
  #par(mfcol=c(1,1))
  par(cex=1.3)
  #par(mar=c(4,4,2,1))
  #par(mgp = c(2, 0.5, 0))
  plot(pr,color="orange",auc.main=1,min.plot=F,max.plot=F,rand.plot=T, fill.area=F, maxminrand.col="blue",legend=F, main='PR curve of Lasso')
  dev.off()
  
  Result=data.frame(SE=SE,SP=SP,ACC=ACC,Fmeasure=Fmeasure,MCC=MCC,AUC=model_roc$auc,AUPRC=pr$auc.integral)
  write.table(Result,file="selectedFeature_LASSO_finalResult_micro_averagePrediction.txt",quote=FALSE,col.names=T,row.names=F,sep='\t')
